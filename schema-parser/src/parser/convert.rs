...
use schema_model::model::types::{KeyType, LockEscalation, RelationType};
use schema_model::model::view::View;
use schema_model::model::column_type::ColumnType;

use super::nodes::*;

pub fn convert_database(db: DatabaseXml) -> Result<Vec<Schema>, String> {
    ...
    for s in db.schemas.into_iter() {
        ...
        for t in s.tables.iter() { sb = sb.add_table(convert_table(t, &s.name)?); }
...
fn convert_table(t: &TableXml, schema_name: &str) -> Result<schema_model::model::table::Table, String> {
    ...
    let mut tb = TableBuilder::new(schema_name, &t.name)
        .no_export(t.no_export.unwrap_or(false));
    if let Some(c) = &t.export_data_column { tb = tb.export_date_column(c.clone()); }
...
fn str_to_database_type_opt(s: Option<&str>) -> Option<schema_model::model::types::DatabaseType> {
    s.and_then(|v| match v.to_ascii_lowercase().as_str() {
        "pgsql" | "postgres" | "postgresql" => Some(schema_model::model::types::DatabaseType::Postgres),
        "mysql" => Some(schema_model::model::types::DatabaseType::Mysql),
        "sqlite" => Some(schema_model::model::types::DatabaseType::Sqlite),
        "mssql" | "sqlserver" => Some(schema_model::model::types::DatabaseType::SqlServer),
        _ => None,
    })
}
