...
**DO NOT commit code changes without explicit user approval.** 

When you complete implementation work:
1. Verify that tests pass
2. Summarize what was implemented
3. Wait for user confirmation before creating a commit

This ensures the user has the final say on what gets committed to the repository.
...
# Test schema-installer migrations
...
schema-installer      → Schema installation utilities + migrations (Flyway-style)
```
...
XML Schema File → [schema-parser] → DatabaseModel → [schema-sql-generator] → SQL
                                  → [schema-installer] → Apply to Database
```
...
### Schema Installer & Migrations

The `schema-installer` crate provides a Flyway-style migration system.

**Core Concepts:**
- Migrations tracked in `schema_migration` table with SHA-256 checksums
- Versions use semantic numbering (1, 1.1, 2.0 sorted numerically)
- Four commands: `migrate`, `info`, `validate`, `repair`
- Supports SQL Server `GO` delimiter, PostgreSQL `;`, SQLite `;`

**Migration Files:**
- Format: `V{version}__{description}.sql` (e.g., `V1__create_users.sql`)
- Loaded from directory, sorted by version, applied in order
- Checksums prevent tampering or accidental modifications

**Key Files:**
- `migration.rs` - Migration types, MigrationSource trait, checksum computation
- `migrator.rs` - migrate/info/validate/repair operations
- `connection.rs` - `schema_migration` table methods
- `tracking.rs` - DDL for single schema_migration table (replaces old tracking tables)
...
**SchemaInstallerConfig** (in `schema-installer`):
- `database_type`: GeneratorType (Postgres, Sqlite, SqlServer)
