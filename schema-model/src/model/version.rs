...
impl Version {
    pub fn new(major_version: i32, minor_version: i32) -> Self {
        Self { major_version, minor_version, patch_version: 0, pre_release_suffix: false }
    }

    pub fn with_patch(major_version: i32, minor_version: i32, patch_version: i32) -> Self {
        Self { major_version, minor_version, patch_version, pre_release_suffix: false }
    }

    pub fn with_patch_and_suffix(major_version: i32, minor_version: i32, patch_version: i32, pre_release_suffix: bool) -> Self {
        Self { major_version, minor_version, patch_version, pre_release_suffix }
    }
    ...
    pub fn parse(version_str: &str) -> Self {
        ...
        let major = parts.get(0).and_then(|p| p.parse::<i32>().ok()).unwrap_or(0);
        let minor = parts.get(1).and_then(|p| p.parse::<i32>().ok()).unwrap_or(0);
        let patch = parts.get(2).and_then(|p| p.parse::<i32>().ok()).unwrap_or(0);
        Self { major_version: major, minor_version: minor, patch_version: patch, pre_release_suffix: pre }
    }

    pub fn major_version(&self) -> i32 { self.major_version }
    pub fn minor_version(&self) -> i32 { self.minor_version }
    pub fn patch_version(&self) -> i32 { self.patch_version }
    pub fn is_pre_release_suffix(&self) -> bool { self.pre_release_suffix }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if self.pre_release_suffix {
            if self.patch_version < 1 {
                write!(f, "{:02}.{:02}-SNAPSHOT", self.major_version, self.minor_version)
            } else {
                write!(f, "{:02}.{:02}.{:02}-SNAPSHOT", self.major_version, self.minor_version, self.patch_version)
            }
        } else if self.patch_version < 1 {
            write!(f, "{:02}.{:02}", self.major_version, self.minor_version)
        } else {
            write!(f, "{:02}.{:02}.{:02}", self.major_version, self.minor_version, self.patch_version)
        }
    }
}
...
impl From<&str> for Version {
    fn from(value: &str) -> Self { Version::parse(value) }
}
