...
mod tests {
    ...
    fn output_relations_renders_foreign_key_constraint() {
        ...
        let child = TableBuilder::new(None::<&str>, "child")
            ...
            .add_relation(Relation::new("parent", "id", "child", "parent_id", RelationType::SetNull, false))
            .build();
        ...
        let generator = PostgresRelationGenerator::new(ctx);
        generator.output_relations();

        let output = buffer.contents();
        assert!(output.contains("alter table public.child"));
        assert!(output.contains("foreign key (parent_id) references public.parent(id)"));
        assert!(output.contains("on delete set null"));
    }
