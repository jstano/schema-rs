...
impl ColumnTypeGenerator for H2ColumnTypeGenerator {
    ...
    fn sequence_sql(&self) -> String {
        "integer generated by default as identity (start with 1)".to_string()
    }

    fn long_sequence_sql(&self) -> String {
        "bigint generated by default as identity (start with 1)".to_string()
    }

    fn text_sql(&self, _column: &Column) -> String {
        "character large object".to_string()
    }

    fn binary_sql(&self) -> String {
        "binary large object".to_string()
    }

    fn uuid_default_value_sql(&self, _schema: &Schema) -> String {
        "random_uuid(7)".to_string()
    }
